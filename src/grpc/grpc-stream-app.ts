// src/grpc/grpc-stream-app.ts - ENHANCED WITH LIQUIDITY + VOLUME ANALYTICS + ARCHIVE v4.28
...
import { ARCHIVE_JOB_SERVICE, ArchiveCompleteEvent, TokenArchivedEvent } from '../services/archive-job-service';
...
export class GrpcStreamApplication {  
  ...
  private setupEventHandlers(): void {  
    ...
    // V4.28: Handle archive completion
    ARCHIVE_JOB_SERVICE.on('archiveComplete', (summary: ArchiveCompleteEvent) => {
      if (summary.tokensArchived > 0) {
        logger.info(`🗄️ Archive job completed: ${summary.tokensArchived} tokens archived`);
      }
    });
    
    // V4.28: Handle individual token archival
    ARCHIVE_JOB_SERVICE.on('tokenArchived', (event: TokenArchivedEvent) => {
      const displaySymbol = event.symbol && event.symbol !== 'LOADING...' 
        ? event.symbol 
        : event.address.substring(0, 8) + '...';
      
      logger.info(`📦 ARCHIVED: ${displaySymbol} - Was ${event.previousCategory}, now below $8k for 48h`);
      
      // Broadcast to WebSocket if enabled
      if (this.wsService) {
        this.wsService.broadcast('tokenArchived', event);
      }
    });
    
    // V4.28: Handle archive errors
    ARCHIVE_JOB_SERVICE.on('error', (error: Error) => {
      logger.error('Archive job error:', error);
    });
  ...
  async start(): Promise<void> {  
    logger.info('🚀 Starting gRPC Stream Application v4.28 with Enhanced Liquidity + Volume Analytics + $8k Filter...');  
    ...
    try {  
      ...
      logger.info('✅ gRPC Stream Application v4.28 started successfully');  
      
      // V4.28: Log configuration summary
  ...
  private async initializeServices(): Promise<void> {  
    ...
    // V4.28: Check archive columns
    const archiveColumnsCheck = await db.raw(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_name = 'tokens'
      AND column_name IN ('first_seen_above_8k', 'below_8k_since', 'archive_reason', 'archived_at')
    `);
    const hasArchiveColumns = archiveColumnsCheck.rows.length === 4;
    logger.info(`✅ Archive tracking columns: ${hasArchiveColumns ? 'Present' : 'Missing - run migration'}`);
    ...
    // V4.28: Start archive job service
    await ARCHIVE_JOB_SERVICE.start();
    logger.info('✅ Archive job service started');
  ...
  private logConfigurationSummary(): void {
    console.log('\n' + '='.repeat(80));
    console.log('📋 CONFIGURATION SUMMARY (v4.28)');
    console.log('='.repeat(80));
    console.log(`🎯 Market Cap Thresholds:`);
    console.log(`   - Minimum to track: $${categoryConfig.thresholds.MIN_MARKET_CAP}`);
    console.log(`   - LOW: $${categoryConfig.thresholds.LOW_MIN} - $${categoryConfig.thresholds.LOW_MAX}`);
    ...
    console.log(`   - AIM (Buy Zone): $${categoryConfig.thresholds.AIM_MIN} - $${categoryConfig.thresholds.AIM_MAX}`);
    console.log(`   - GRADUATED: > $${categoryConfig.thresholds.AIM_MAX}`);
    console.log(`🗄️ Archive Settings:`);
    console.log(`   - Archive after: ${categoryConfig.archiveSettings.belowThresholdHours} hours below $8k`);
    console.log(`   - Check interval: ${categoryConfig.archiveSettings.checkIntervalMinutes} minutes`);
    console.log('='.repeat(80) + '\n');
  }
  ...
  private startPeriodicTasks(): void {  
    ...
    this.statsInterval = setInterval(async () => {  
      ...
      // V4.28: Get archive stats
      const archiveStats = await ARCHIVE_JOB_SERVICE.getArchiveStats();
      ...
      logger.info('📊 Stream Statistics:', {  
        ...
        archiveStats: {
          ...
          last24h: archiveStats.archivedLast24h,
          candidates: archiveStats.currentCandidates
        }
      });
  ...
  private async checkHealth(): Promise<any> {  
    ...
    const volumeStats = VOLUME_ANALYTICS_SERVICE.getStats();  
    // V4.28: Check archive service health
    const archiveStats = await ARCHIVE_JOB_SERVICE.getArchiveStats();
    ...
    // V4.28: Check archive service health
    const archiveHealthy = archiveStats.isRunning;
      
    const healthy = dbHealthy && stats.grpcConnected && dataFresh && metadataHealthy &&   
                   holderAnalyticsHealthy && liquidityAnalyticsHealthy && volumeAnalyticsHealthy && archiveHealthy;  
  ...
  private setupGracefulShutdown(): void {  
    const shutdown = async (signal: string) => {  
      ...
      try {  
        ...
        // V4.28: Stop archive job service
        await ARCHIVE_JOB_SERVICE.stop();
  ...
  // V4.28: Archive management methods
  async getArchiveStats(): Promise<any> {
    return await ARCHIVE_JOB_SERVICE.getArchiveStats();
  }
  
  async getTokensBelowThreshold(): Promise<any[]> {
    return await ARCHIVE_JOB_SERVICE.getTokensBelowThreshold();
  }
  
  async runManualArchive(): Promise<number> {
    logger.info('🔧 Running manual archive check...');
    return await ARCHIVE_JOB_SERVICE.manualArchiveCheck();
  }
